...
export function sumArrayWithThreeNumbers(sumArr) {
    ...
    const numB = sumArr[1];
    const numC = sumArr[2];
    const numD = sum(numA, numB)[0];
    const numE = sum(numC, numD)[0];
        
    return [
        numE,
        `${numA},${numB},${numC} was passed in as an array of numbers, and ${numE} is their sum.`
    ];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    ...
    const numF = multArr[0];
    const numG = multArr[1];
    const numH = multArr[2];
    const numI = multiply(numF, numG)[0];
    const numJ = multiply(numH, numI)[0];
        
    return [
        numJ,
        `The numbers ${numF},${numG},${numH} have a product of ${numJ}.`
    ];
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    const dynA = dynamicArray[0];
    const dynB = dynamicArray[1];
    const dynC = dynamicArray[2];
    ...
    for(let i = 0; i < dynamicArray.length; i++) {
        ...
        return [
            dynArray,
            `The numbers ${dynA},${dynB},${dynC},${dynD},${dynE} have a product of 120.`
        ];
    }
}
